...
Sprite* AnimationTool::startFireAnm(Point &pos,const char * image,const char * plist,const char * imgSplit,int count,const char *format,Object* pSelectorTarget,SEL_CallFuncND selector){
    
    TextureCache::sharedTextureCache()->addImage(image);
    TextureCache *cache = CCTextureCache::sharedTextureCache();
    Texture2D *texture2D = cache->textureForKey(image);
    SpriteFrameCache::sharedSpriteFrameCache()->addSpriteFramesWithFile(plist, texture2D);
    Vector normalAnimFrames = Vector::create();
    for (int i=1; i<count; i++) {
        char* tempString = new char;
        sprintf(tempString, format, imgSplit,i);
        SpriteFrame *frame = SpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName(tempString);
        normalAnimFrames->addObject(frame);
    }
    Animation *animation = Animation::createWithSpriteFrames(normalAnimFrames,0.1f);
